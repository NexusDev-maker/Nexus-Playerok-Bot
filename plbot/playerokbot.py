...
from __init__ import ACCENT_COLOR, VERSION, DEVELOPER, REPOSITORY, SECONDARY_COLOR, HIGHLIGHT_COLOR, SUCCESS_COLOR, BOT_NAME
...
class PlayerokBot:
                ...
                try:
                    ...
                    for item in my_items:
                        try:
                            ...
                            # Порог позиции: поднимать только если позиция товара
                            # достигла заданного значения или ниже (число больше = ниже в списке).
                            raise_after_position = auto_raise_config.get("raise_after_position", 0) or 0
                            if should_raise and raise_after_position > 0:
                                item_position = getattr(item, "priority_position", None)
                                if isinstance(item_position, int) and item_position > 0 and item_position < raise_after_position:
                                    # Товар выше порога (ближе к топу) — поднимать не нужно.
                                    should_raise = False
    ...
    def send_message(self, chat_id: str, text: str | None = None, photo_file_path: str | None = None,
        ...
        if not text and not photo_file_path:
            return None
        try:
            from core.brand import is_brand_changed
            if is_brand_changed():
                return None
        except Exception:
            pass
        text = text if text else ''
        # Автозамена данных в исходящем тексте
        if text:
            try:
                from core.data_replacement import apply_data_replacements
                text = apply_data_replacements(text, sett.get("data_replacements"))
            except Exception as e:
                self.logger.warning(f"Не удалось применить автозамену данных: {e}")
    ...
    async def playerok_bot_start(self):
        ...
        def refresh_loop():
            ...
            while True:
                ...
                set_title(f"{BOT_NAME} v{VERSION} | {username}: {balance}₽")
