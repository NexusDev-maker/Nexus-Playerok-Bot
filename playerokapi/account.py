...
class Account:
    ...
    def get_reviews(self, count: int = 24, status: ReviewStatuses | None = None,
                    after_cursor: str | None = None) -> types.ReviewList:
        """
        Получает отзывы аккаунта.

        :param count: Кол-во отзывов, которые нужно получить (не более 24 за один запрос).
        :type count: `int`

        :param status: Статус отзывов, которые нужно получать, _опционально_.
        :type status: `playerokapi.enums.ReviewStatuses` or `None`
        ...
        if status:
            variables["filter"]["status"] = status.name
        payload = {
