...
from __init__ import SKIP_UPDATES, VERSION, REPO, REPO_CONFIGURED
...
def _fetch_releases() -> list[dict]:
    headers = {"User-Agent": "PlayerokBot-Updater"}
...
def get_update_status() -> dict[str, Any]:
    ...
    # Репозиторий ещё не настроен (стоит заглушка) — не дёргаем GitHub.
...
def install_release_update(release_info: dict, restart_on_success: bool = True) -> dict[str, Any]:
    ...
    - error
    """
    latest_version = release_info.get("tag_name") if isinstance(release_info, dict) else None
    result: dict[str, Any] = {
        "status": "error",
