...
async def handler_start(message: types.Message, state: FSMContext):
    ...
    try:
        from core.brand import is_brand_changed, TAMPER_MESSAGE
        if is_brand_changed():
            await message.answer(TAMPER_MESSAGE, parse_mode="HTML")
    except Exception:
        pass
...
async def handler_update(message: types.Message, state: FSMContext):
    ...
    await _run_update_flow(message, state)


@router.callback_query(F.data == "run_update")
async def callback_run_update(callback: types.CallbackQuery, state: FSMContext):
    """
    Кнопка «Обновления» в меню — делает то же, что и команда /update.
    """
    config = sett.get("config")
    ...
    await _run_update_flow(callback.message, state)


async def _run_update_flow(message: types.Message, state: FSMContext):
    """Общий процесс проверки и установки обновления (для команды и кнопки)."""
    await state.set_state(None)
...
async def handler_watermark(message: types.Message, state: FSMContext):
    """
    Обработчик команды /watermark
    Показывает варианты водяного знака и позволяет его убрать (ввод «-»).
    """
    ...
    await state.set_state(st.SettingsStates.waiting_for_watermark_value)
    await throw_float_message(
        state=state,
        message=message,
