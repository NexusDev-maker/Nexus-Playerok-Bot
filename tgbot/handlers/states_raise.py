...
@router.message(states.RaiseItemsStates.waiting_for_raise_after_position, F.text)
async def handler_waiting_for_raise_after_position(message: types.Message, state: FSMContext):
    try:
        await state.set_state(None)
        msg_text = message.text.strip()

        if not msg_text.isdigit():
            raise Exception("❌ Введите целое число (0 — выключить)")
        position = int(msg_text)
        if position < 0:
            raise Exception("❌ Значение не может быть отрицательным")
        if position > 100000:
            raise Exception("❌ Слишком большое значение")

        config = sett.get("config")
        config["playerok"]["auto_raise_items"]["raise_after_position"] = position
        sett.set("config", config)

        if position > 0:
            done_text = f"✅ Поднятие с позиции установлено: <b>{position}</b>."
        else:
            done_text = "✅ Проверка позиции выключена (поднятие по интервалу/таймингу)."

        await throw_float_message(
            state=state,
            message=message,
            text=templ.settings_raise_float_text(done_text),
            reply_markup=templ.back_kb(calls.SettingsNavigation(to="raise").pack())
        )
    except Exception as e:
        await throw_float_message(
