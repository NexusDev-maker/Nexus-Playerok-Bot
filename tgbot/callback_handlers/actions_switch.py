...
@router.callback_query(F.data == "set_auto_raise_after_position")
async def callback_set_auto_raise_after_position(callback: CallbackQuery, state: FSMContext):
    config = sett.get("config")
    current = config["playerok"]["auto_raise_items"].get("raise_after_position", 0) or 0
    current_text = f"<b>{current}</b>" if current > 0 else "выключено"

    await state.set_state(states.RaiseItemsStates.waiting_for_raise_after_position)

    await throw_float_message(
        state=state,
        message=callback.message,
        text=f"📍 <b>Поднятие с позиции</b>\n\n"
             f"Текущее значение: {current_text}\n\n"
             f"Введите номер позиции ↓\n\n"
             f"💡 <i>Бот будет поднимать товар, когда его позиция достигнет этого числа или ниже "
             f"(1 — самый верх; чем больше число, тем ниже товар).\n"
             f"Введите <b>0</b>, чтобы выключить проверку позиции.</i>",
        reply_markup=templ.back_kb(calls.SettingsNavigation(to="raise").pack())
    )
...
async def callback_set_watermark(callback: CallbackQuery, callback_data: calls.SetWatermark, state: FSMContext):
    ...
    config["playerok"]["watermark"]["enabled"] = bool(callback_data.value)
    sett.set("config", config)
    await callback.answer(f"✅ Водяной знак установлен: {callback_data.value}", show_alert=True)
    return await callback_settings_navigation(callback, calls.SettingsNavigation(to="watermark"), state)


@router.callback_query(calls.WatermarkPreset.filter())
async def callback_watermark_preset(callback: CallbackQuery, callback_data: calls.WatermarkPreset, state: FSMContext):
    from ..templates.settings_watermark import WATERMARK_PRESETS
    index = callback_data.index
    if index < 0 or index >= len(WATERMARK_PRESETS):
        await callback.answer("❌ Вариант не найден.", show_alert=True)
        return
    value = WATERMARK_PRESETS[index]
    config = sett.get("config")
    config["playerok"]["watermark"]["value"] = value
    config["playerok"]["watermark"]["enabled"] = True
    sett.set("config", config)
    await callback.answer(f"✅ Установлен: {value}", show_alert=True)
    return await callback_settings_navigation(callback, calls.SettingsNavigation(to="watermark"), state)


@router.callback_query(F.data == "watermark_disable")
async def callback_watermark_disable(callback: CallbackQuery, state: FSMContext):
    config = sett.get("config")
    config["playerok"]["watermark"]["enabled"] = False
    sett.set("config", config)
    await callback.answer("✅ Водяной знак убран.", show_alert=True)
    return await callback_settings_navigation(callback, calls.SettingsNavigation(to="watermark"), state)
