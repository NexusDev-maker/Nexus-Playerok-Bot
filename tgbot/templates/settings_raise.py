...
def settings_raise_text():
    ...
    position_line = (
        f"📍 <b>Поднимать с позиции:</b> {raise_after_position}"
        if raise_after_position > 0
        else "📍 <b>Поднимать с позиции:</b> выключено"
    )
...
def settings_raise_kb():
    ...
    raise_after_position = auto_raise_config.get("raise_after_position", 0) or 0
    position_label = f"с позиции {raise_after_position}" if raise_after_position > 0 else "выключено"
    ...
    rows.extend([
        ...
        [InlineKeyboardButton(text=f"📍 Поднимать: {position_label}", callback_data="set_auto_raise_after_position")],
