...
from __init__ import VERSION, DEVELOPER, REPOSITORY, BOT_NAME, TELEGRAM_CHAT
...
def start_banner_caption_text() -> str:
    txt = textwrap.dedent("""
        ...
        <i>🦅 Powered by Nexus Playerok Bot • @NexusPlayerok</i>
    """)
    return txt.strip()
...
def menu_kb(page: int = 0):
    ...
    if page == 0:
        ...
        rows = [
            ...
            [InlineKeyboardButton(text="💰 Финансы", callback_data=calls.FinanceNavigation(to="main").pack())],
            [InlineKeyboardButton(text="⭐ Отзывы", callback_data=calls.ReviewsNavigation(to="main").pack())],
            [InlineKeyboardButton(text="🔁 Замена данных", callback_data=calls.DataReplacementsNavigation(to="main").pack())],
            ...
            [InlineKeyboardButton(text="🔄 Обновления", callback_data="run_update")],
    ...
    # Ссылки
    rows.append([
        InlineKeyboardButton(text="👨‍💻 Связь", url=TELEGRAM_CHAT)
    ])
